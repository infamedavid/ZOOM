...
def _calculate_grab_speed_multiplier():
    ...
    if project_duration <= 0 or GRAB_PROJECT_LENGTH_DIVISOR <= 0:
        project_factor = 1.0
    else:
        project_factor = 1.0 + (project_duration / GRAB_PROJECT_LENGTH_DIVISOR)
...
def _perform_translate_from_jog(jog_value):
    ...
    if not cs.get('snap_active', False) or 'translate' not in cs.get('active_tools', set()):
        frame_delta = jog_value * GRAB_BASE_SENSITIVITY * speed_multiplier
    ...
    if not ss['candidates']:
        perform_translate_timeline(jog_value * GRAB_BASE_SENSITIVITY * speed_multiplier)
        return
...
def _perform_offset_from_jog(jog_value, property_name):
    ...
    if state.control_state.get('shift_active', False):
        speed_multiplier /= OFFSET_PRECISION_DIVISOR
    frame_delta = jog_value * OFFSET_JOG_SENSITIVITY * speed_multiplier
