...
def _perform_speed_from_jog(jog_value):
    ...
    sensitivity = 1.0
    if method == 'MULTIPLY':
        sensitivity = config.SPEED_FACTOR_SENSITIVITY_JOG
    elif method == 'STRETCH':
        sensitivity = config.SPEED_STRETCH_SENSITIVITY_JOG
    else: # FRAME_NUMBER y LENGTH
        sensitivity = config.SPEED_FRAME_LENGTH_SENSITIVITY_JOG
...
def _perform_speed_from_plus_minus(direction):
    ...
    sensitivity = 1.0
    if method == 'MULTIPLY': sensitivity = config.SPEED_FACTOR_SENSITIVITY_NUDGE
    elif method == 'STRETCH': sensitivity = config.SPEED_STRETCH_SENSITIVITY_NUDGE
    else: sensitivity = config.SPEED_FRAME_LENGTH_SENSITIVITY_NUDGE
...
def _perform_wipe_from_jog(jog_value):
    ...
    sensitivity = config.WIPE_SENSITIVITY_JOG
    if active_tool == "wipe_angle":
        sensitivity = config.WIPE_ANGLE_SENSITIVITY_JOG
...
def _perform_wipe_from_plus_minus(direction):
    ...
    sensitivity = config.WIPE_SENSITIVITY_NUDGE
    if active_tool == "wipe_angle":
        sensitivity = config.WIPE_ANGLE_SENSITIVITY_NUDGE
...
FX_TOOL_PROPERTIES = {
    ...
    # Transform
    "transf_pos_x":   {'prop': 'translate_start_x', 'strip': 'TRANSFORM', 'type': 'float', 'sens_jog': config.TRANSFORM_SENSITIVITY_JOG, 'sens_nudge': config.TRANSFORM_SENSITIVITY_NUDGE},
    "transf_pos_y":   {'prop': 'translate_start_y', 'strip': 'TRANSFORM', 'type': 'float', 'sens_jog': config.TRANSFORM_SENSITIVITY_JOG, 'sens_nudge': config.TRANSFORM_SENSITIVITY_NUDGE},
    "transf_scale_x": {'prop': 'scale_start_x', 'strip': 'TRANSFORM', 'type': 'float', 'sens_jog': config.TRANSFORM_SENSITIVITY_JOG, 'sens_nudge': config.TRANSFORM_SENSITIVITY_NUDGE},
    "transf_scale_y": {'prop': 'scale_start_y', 'strip': 'TRANSFORM', 'type': 'float', 'sens_jog': config.TRANSFORM_SENSITIVITY_JOG, 'sens_nudge': config.TRANSFORM_SENSITIVITY_NUDGE},
    "transf_rot":     {'prop': 'rotation_start', 'strip': 'TRANSFORM', 'type': 'float', 'sens_jog': config.TRANSFORM_ROT_SENSITIVITY_JOG, 'sens_nudge': config.TRANSFORM_ROT_SENSITIVITY_NUDGE, 'is_degrees': True},
...
def _perform_fx_from_jog(jog_value):
    ...
    sensitivity = details.get('sens_jog', config.GENERIC_FX_SENSITIVITY_JOG)
    if state.control_state.get('shift_active', False):
        sensitivity /= config.PRECISION_DIVISOR
    delta = jog_value * sensitivity
...
def _perform_fx_from_plus_minus(direction):
    ...
    sensitivity = details.get('sens_nudge', config.GENERIC_FX_SENSITIVITY_NUDGE)
    delta = direction * sensitivity
